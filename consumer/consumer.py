...
if __name__ == "__main__":
    ...
    stockDataframe.writeStream \
        ...
        .start()
    stockDataframe.selectExpr("CAST(value AS STRING)").writeStream.format("console").start()
    ...
    logger.info("After parsing JSON data into schema")
    stockDataframe.writeStream \
        .format("console") \
        .option("truncate", "false") \
        .start()
    ...
    logger.info("Expanded DataFrame to select individual fields")
    expandedDf.writeStream \
            .format("console") \
            .outputMode("append") \
            .start()
    ...
    def process_batch(batch_df, batch_id):
        ...
        for realtimeStockPrice in realtimeStockPrices.collect():
            ...
            try:
                logger.info(f"Writing to InfluxDB: timestamp={timestamp}, tags={tags}, fields={fields}")
